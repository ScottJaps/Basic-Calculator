...
public class Calculator {
	...
	//this function helps keep strings in the correct format when new things are added
	private void addToCurrentString(String x){
		...
		else if(x == "-") currentInt = "-" + currentInt;												//adds negitive sign
		else currentInt = currentInt + x;
	}
	
	
	// This function draws the calculator, including updating the displayed text.
	...
	public void drawCalculator(){
        ...
        //the priority is to display currentInt, but if it is not set yet, to display previousInt
	...
	// factorial for use in sin and cos, tail recursive
	private double factorial(double x, double acc){                          
		if (x == 1) return acc;
		else if (x>1)  return factorial(x-1, x*acc);
		else return 0;
	}
	
	//my own sine function, tail recursive
	private double findSinIter(double x, double acc, double exp){            
		double next;
		...
		//Is it close enough to the real answer?
	...
	//my own cosine function, tail recursive
	private double findCosIter(double x, double acc, double exp){              
		double next;
		//find the next element in the series
		if ((exp) % 4 == 0) {     //add next 
			next = Math.pow(x, exp)/factorial(exp,1);
	...
	//is there enough information to evaluate this?
	private boolean canEvaluate(){
		if(operator != null && currentInt != null && previousInt != null) return true;
		else return false;
	}
	
	//evaluates the input with basic operations, you can't use switch statements with strings in java 1.6
	...
	//gets the input and adds it to the holding strings depending on where the user clicked
	public void inputs(double x, double y){
		if (x > 0 && x < 200 && y > 0 && y < 150){                              //0
			...
			else if(currentInt == null) operator = null;  
